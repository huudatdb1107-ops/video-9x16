...
### Pipeline 3 pha (tách biệt rõ ràng)

```
── PHA 1: HTML ──────────────────────────────────────────────── [AI Agent tự làm]
   script.txt  →  AI Agent fill content.json  →  compose.py  →  index.html
                                                                      ↓
                                                               MO_EDITOR.vbs   ← LUÔN tạo ngay sau html

── PHA 2: VOICE ─────────────────────────────────────────────── [Độc lập, sau Pha 1]
   script.txt  →  gen_voice.py (OMNI)  →  TT_*.wav
   TT_*.wav    →  faster-whisper        →  transcript.json
   transcript  →  update_timeline       →  cập nhật element_times trong content.json

── PHA 3: VIDEO ─────────────────────────────────────────────── [Boss tự làm]
   MO_EDITOR.vbs  →  double-click  →  🎬 Gen Video  →  .mp4
```

### Quy tắc thực thi

1. **AI Agent = người fill content.json** — Bất kỳ AI agent nào chạy skill (Claude, Gemini, Antigravity...) đều tự đọc `script.txt` + `content.schema.json` → fill `content.json` trực tiếp. KHÔNG phụ thuộc `agy_wrapper.py`. `fill_content.py` chỉ dùng khi chạy pipeline CLI.

2. **Sau khi có `index.html` → LUÔN tạo `MO_EDITOR.vbs`** — không có ngoại lệ. VBS đồng bộ với html để Boss preview ngay.

3. **Pha 2 và Pha 3 là độc lập** — Không bắt buộc chạy sau Pha 1. Boss có thể dùng html trước, gen voice sau, render video khi sẵn sàng.

4. Khi Boss gõ trigger → hỏi 2 thông tin:
   - **Brand** (Kênh đăng — đọc từ `video_brand_profiles.json`: `vicon`, `gocnho`, `bsimple`)
   - **Script hoặc Chủ đề** (file script.txt có sẵn, hoặc chủ đề để AI tự viết)

   > **👶 BRAND KID (Vì Con không thể đợi) — QUY CHẾ VIẾT KỊCH BẢN:**
   > Kịch bản phải dài ~187 từ, chia 5 phần (Hook, Khoa học, 3 Điểm dạng Một/Hai/Ba, Câu thấu cảm in nghiêng, CTA chuẩn cố định). Đọc `references/master_prompt_kid.md` trước khi viết.

   > **🚫 SCRIPT FORMAT — BẮT BUỘC:**
   > Script là văn đọc thẳng cho TTS — KHÔNG có label "Cảnh 1:", "Scene 1:", hay bất kỳ tiêu đề nào.
   > ✅ ĐÚNG: "Em bé 7 tuổi năng động, nhưng khi đến trường lại gặp khó khăn..."
   > ❌ SAI: "Cảnh 1: Em bé 7 tuổi..." / "Bộ cảnh: Một em bé..."

5. **Output Pha 1**: folder chứa `content.json` + `index.html` + `MO_EDITOR.vbs` — Boss double-click VBS để preview/chỉnh ngay.
...
2. **KHÔNG dump file ra root của page folder** — KHÔNG tạo `content.json`, `script.txt`, `llm_raw_response.txt` ở `E:\HuuDat\VIDEO\03_KID\` trực tiếp. Mọi file phải nằm trong SUBFOLDER output.

3. **`--output-dir` PHẢI là subfolder trong page folder** — ví dụ `E:\HuuDat\VIDEO\03_KID\` (để `run_pipeline.py` tự tạo subfolder với timestamp). KHÔNG dùng `--output-dir` trỏ thẳng vào root page folder.
...
python scripts/run_pipeline.py \
  ...
  --page vicon
```
*(Hệ thống tự động ánh xạ thư mục Vì Con `F:\VIDEO\09_POST\FACEBOOK\03_KID`, áp dụng giọng đọc `TT_06`, watermark `PK NHI BOOM BOOM`, template `01_Text` và hashtag `#NuoiDayCon`)*

**Các Page Profile được cấu hình động trong `video_brand_profiles.json`:**
- `vicon` (👶 Vì Con không thể đợi): `E:\HuuDat\VIDEO\03_KID` (Giọng `TT_06`, Watermark `PK NHI BOOM BOOM`, Hashtag `#NuoiDayCon`)
- `gocnho` (📚 Góc nhỏ - Sách & Đời): `E:\HuuDat\VIDEO\02_BOOK` (Giọng `TT_01`, Watermark `GÓC NHỎ`, Hashtag `#Stoic`)
- `bsimple` (🚀 B.Simple): `E:\HuuDat\VIDEO\01_B.Simple` (Giọng `TT_02`, Watermark `B.SIMPLE`, Hashtag `#AI`)
